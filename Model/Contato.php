...
class Contato
{
    ...
    {
        /*
         * Aqui abaixo você poderia criar um arquivo com nome diferente. Ex: 'contatos.ini' e chamar como 'contatos'
         */
        $conexao = Conexao::getInstancia('config');
        if ($this->id) {
    ...
    public static function carregar(int $id): Contato
    {
        ...
        if ($stmt->execute()) {
            if ($stmt->rowCount() > 0) {
                ...
                if (!is_null($recordSet->sobrenome))
                    $contato->setSobrenome($recordSet->sobrenome);
                if (!is_null($recordSet->email))
                    $contato->setEmail($recordSet->email);
                if (!is_null($recordSet->telefone))
                    $contato->setTelefone($recordSet->telefone);
                if (!is_null($recordSet->celular))
                    $contato->setCelular($recordSet->celular);
        ...
        if ($exec->execute()) {
            ...
            if ($exec->rowCount() > 0) {
                ...
                foreach ($exec->fetchAll(PDO::FETCH_OBJ) as $recorset) {
                    ...
                    if (!is_null($recorset->sobrenome))
                        $tmpContato->setSobrenome($recorset->sobrenome);
                    if (!is_null($recorset->email))
                        $tmpContato->setEmail($recorset->email);
                    if (!is_null($recorset->telefone))
                        $tmpContato->setTelefone($recorset->telefone);
                    if (!is_null($recorset->celular))
                        $tmpContato->setCelular($recorset->celular);
